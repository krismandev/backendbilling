...
import (
	...
	"billingdashboard/modules/config/services"
	"context"
	"errors"
...
// ListConfigEndpoint is as request middleware
func ListConfigEndpoint(conn *connections.Connections) endpoint.Endpoint {
	return func(ctx context.Context, request interface{}) (interface{}, error) {
		...
		if req, ok := request.(dt.ConfigRequest); ok {
			return services.ListConfig(ctx, req, conn), nil
		}
