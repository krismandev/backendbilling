...
type DatabaseConnection interface {
	...
	// Added by Christian
	/* For slice datatype with ordering support */
	GetRowsSlice(rows *sql.Rows) ([]map[string]string, int, error)
	SelectQuerySlice(strSQL string, args ...interface{}) ([]map[int]string, int, error)
	GetRowsbyIndexSlice(rows *sql.Rows) ([]map[int]string, int, error)
	GetFirstRowByQuerySlice(strSQL string, args ...interface{}) ([]map[int]string, error)
	SelectQueryByFieldNameSlice(strSQL string, args ...interface{}) ([]map[string]string, int, error)
	GetCustomRowColumnSlice(rows *sql.Rows, maxRow int, maxColumn int) ([]map[int]string, error)
}
...
// return example
// map[1:map[0:1 1:USAGE 2:Krisman 3:NULL]]
...
/*
Use below methods if you need select query function that need data ordering
*/

//GetRowsSlice parses recordset into map
...
//SelectQuerySlice parse and gets column value in first record by query
func (c DBConnection) SelectQuerySlice(strSQL string, args ...interface{}) ([]map[int]string, int, error) {
	var rowret *sql.Rows
	var err error
	if rowret != nil {
		defer rowret.Close()
	...
	results, rowCount, err := c.GetRowsbyIndexSlice(rowret)
	if err != nil {
		return nil, 0, err
	}
	return results, rowCount - 1, nil
...
//GetRowsbyIndexSlice parses Get row using index of row and column
func (c DBConnection) GetRowsbyIndexSlice(rows *sql.Rows) ([]map[int]string, int, error) {
...
//GetCustomRowColumnSlice parses Get any number of  row or column where 0 = get all
//maxRow,maxColumn = row/column count, start from
func (c DBConnection) GetCustomRowColumnSlice(rows *sql.Rows, maxRow int, maxColumn int) ([]map[int]string, error) {
	var results []map[int]string
	if rows != nil {
		defer rows.Close()
	}
...
//GetFirstRowByQuerySlice parse and gets column value in first record by query
func (c DBConnection) GetFirstRowByQuerySlice(strSQL string, args ...interface{}) ([]map[int]string, error) {
	var rowret *sql.Rows
	if rowret != nil {
		defer rowret.Close()
	}
	...
	results, err := c.GetCustomRowColumnSlice(rowret, 1, 0)
	if err != nil {
		return nil, err
	}
